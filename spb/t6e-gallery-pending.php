...
$_SESSION['lastT6eGalleryPage'] = "t6e-gallery-pending.php?by=$by&dir=$dir&p=$p";
...
$totalItemCount = $db->scalar("SELECT count(templateId) FROM Templates WHERE reviewState = 'p'", array(), array());
$page = max(!is_null($p) ? $p : 1, 1);

$startIndex = ($page-1) * $ITEMS_PER_PAGE;
$query = "SELECT t.* FROM Users as u, Templates as t LEFT OUTER JOIN TemplateRatings as r ON r.templateId = t.templateId WHERE t.userId = u.userId AND reviewState = 'p' GROUP BY t.templateId $orderBy $dir LIMIT ? OFFSET ?";
