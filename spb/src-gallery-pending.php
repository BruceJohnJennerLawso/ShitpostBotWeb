...
$_SESSION['lastSrcGalleryPage'] = "src-gallery-pending.php?by=$by&dir=$dir&p=$p";
...
$totalItemCount = $db->scalar("SELECT count(sourceId) FROM SourceImages WHERE reviewState = 'p'", array(), array());
$page = max(!is_null($p) ? $p : 1, 1);

$startIndex = ($page-1) * $ITEMS_PER_PAGE;
$query = "SELECT s.* FROM Users as u, SourceImages as s LEFT OUTER JOIN SourceRatings as r ON  r.sourceId = s.sourceId WHERE s.userId = u.userId AND reviewState = 'p' GROUP BY s.sourceId  $orderBy $dir LIMIT ? OFFSET ?";
